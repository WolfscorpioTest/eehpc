...
/* Inputs */
L2_MEM unsigned char Input_1[79056];
/* Outputs */
L2_MEM signed char Output_1[1];
...
static void cluster()
{
    ...
    #endif
//higher than 218-222
//replace input with img buff array
//right ouput1 to serial port for stm32, lower then forward, higher then pause
    ptq_int8CNN(Input_1, Output_1);
    if(Output_1 <220){
...
void camera_task(void *parameters)
{
    ...
    printf("Constructor\n");
    int ConstructorErr = ptq_int8CNN_Construct();
    if (ConstructorErr)
    {
        printf("Graph constructor exited with error: %d\n(check the generated file ptq_int8Kernels.c to see which memory have failed to be allocated)\n", ConstructorErr);
        pmsis_exit(-6);
    }
